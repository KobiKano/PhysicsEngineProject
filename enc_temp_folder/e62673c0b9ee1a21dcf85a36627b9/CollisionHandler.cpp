...
void CollisionHandler::findPossibleCollisions() {
	...
	for (int j = 0; j < possibleCollisionsCopy.size(); j++) {
		Container container(possibleCollisionsCopy[j]);
		for (float i = -5.0f; i < 5.0f; i += 0.5f) {
			//initialize container
			container.lowerBoundY = i;
			container.upperBoundY = i + 0.5f;
			for (int k = 0; k < possibleCollisionsCopy[j].objects.size(); k++) {
				logger.debugLog("possibleCollisions size: " + std::to_string(possibleCollisionsCopy[j].objects.size()) + "\n");

				//check if any part of object falls within bounds
				if (container.objects[k]->getCenterPos()[1] > i && container.objects[k]->getCenterPos()[1] < i + 0.5f);
				else if (container.objects[k]->getCenterPos()[1] + container.objects[k]->getRadius()[1] > i && container.objects[k]->getCenterPos()[1] + container.objects[k]->getRadius()[1] < i + 0.5f);
				else if (container.objects[k]->getCenterPos()[1] - container.objects[k]->getRadius()[1] > i && container.objects[k]->getCenterPos()[1] - container.objects[k]->getRadius()[1] < i + 0.5f);
				
				//remove object from container if not in bounds
				else {
					container.objects.erase(container.objects.begin() + k);
					k--;
					logger.debugLog("possibleCollisions size: " + std::to_string(possibleCollisionsCopy[j].objects.size()) + "\n");
				}
			}
	...
	for (int j = 0; j < possibleCollisionsCopy.size(); j++) {
		...
		for (float i = -5.0f; i < 5.0f; i += 0.5f) {
			//initialize container
			container.lowerBoundZ = i;
			container.upperBoundZ = i + 0.5f;
			for (int k = 0; k < possibleCollisionsCopy[j].objects.size(); k++) {
				logger.debugLog("possibleCollisions size: " + std::to_string(possibleCollisionsCopy[j].objects.size()) + "\n");
				//check if any part of object falls within bounds
				if (container.objects[k]->getCenterPos()[2] > i && container.objects[k]->getCenterPos()[2] < i + 0.5f);
				else if (container.objects[k]->getCenterPos()[2] + container.objects[k]->getRadius()[2] > i && container.objects[k]->getCenterPos()[2] + container.objects[k]->getRadius()[2] < i + 0.5f);
				else if (container.objects[k]->getCenterPos()[2] - container.objects[k]->getRadius()[2] > i && container.objects[k]->getCenterPos()[2] - container.objects[k]->getRadius()[2] < i + 0.5f);
	...
	for (int i = 0; i < possibleCollisions.size(); i++) {
		if (possibleCollisions[i].objects.size() < 2) {

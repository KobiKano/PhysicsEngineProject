...
void createSphereObject(GLFWwindow* window, PhysicsEngine& physicsEngine) {
	...
	float randX = static_cast <float> (rand()) / static_cast <float> (RAND_MAX / 3.0f) + 2.0f;
	float randY = static_cast <float> (rand()) / static_cast <float> (RAND_MAX / 3.0f) + 2.0f;
	float randZ = static_cast <float> (rand()) / static_cast <float> (RAND_MAX / 3.0f) + 2.0f;

	//add to physics engine
	float centerPos[3] = {randX, randY, randZ};
	float mass = 1.0f;
	...
	//add rotation force
	float direction[3] = { -randX, -randY, -randZ };  //vector pointing to (0,0,0)
	physicsEngine.addForce("object" + std::to_string(objectsAdded), Force::PHYSICS_ROTATIONAL, 9.8f, direction);
	
	//set initial velocity
	float distance = sqrtf(powf(centerPos[0], 2) + powf(centerPos[0], 2) + powf(centerPos[0], 2));
	float velocityMag = 10.0f;
	float theta = atanf(centerPos[2] / centerPos[0]);

	float velocity[3] = {velocityMag * sinf(theta), 0.0f, velocityMag * cosf(theta)};
...
Render::Render(GLFWwindow* window) {
	...
	PhysicsBall sun = PhysicsBall(1.0f, window, "sun");
	float centerPos[3] = { 0.0f, 0.0f, 0.0f };
	float radius[3] = { 1.0f, 1.0f, 1.0f };
	glm::vec3 position;
	glm::vec3 rotation;
	physicsEngine.registerObject(centerPos, radius, "sun", PhysicsObject::PHYSICS_STATIC, PhysicsObject::PHYSICS_CUBE, 10.0f);

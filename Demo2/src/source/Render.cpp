...
void createSphereObject(GLFWwindow* window, PhysicsEngine& physicsEngine) {
	...
	float centerPos[3] = {0.7f, 0.5f, 0.0f};
	float mass = 1.0f;
	...
	//add normal force
	direction[0] = cosf(75.0f * 3.14f / 180.0f);  //convert to radians
	direction[1] = sinf(75.0f * 3.14f / 180.0f);
	float magnitude = -9.8f * cosf(15.0f * 3.14f / 180.0f);
	physicsEngine.addForce("object" + std::to_string(objectsAdded), Force::PHYSICS_NORMAL, magnitude, direction);
...
Render::Render(GLFWwindow* window) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		model = glm::rotate(model, 15.0f * 3.14f / 180.0f, rotation);
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		floor.draw();
